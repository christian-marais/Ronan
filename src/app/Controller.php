<?php
    ...
    abstract class Controller{
        ...
        protected function gestionfichier($data){// Sert pour obtenir les informations de fichiers, à déplacer un fichier ou 
            ...
            $this->file['type']=substr($_FILES['file']['type'],6);//on calcule le type de fichier
            ...
            if(isset($_SESSION['pick'])){//si on a selectionné un fichier
                ...
                $this->pathFileTemp=$this->pathFileTemp.substr($_FILES['file']['type'],6);
            }
            ...
            $ifWgExt=(!in_array($this->file['type'],$data['ext']))?"L'extension doit être un jpg. ":"";
            $_POST['nomImage']=$this->file['type'];
            ...
            if(empty($ifWgExt)&&empty($ifWgDim)&&empty($ifWgSize)){//si message d'erreurs vide donc pas d'erreurs on met un message de succès dans un post 
                ...
                $this->file['new_path']=ROOT.$data['path'].strtolower($this->file['name']).$this->file['type'];
